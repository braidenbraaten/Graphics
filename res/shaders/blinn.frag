...
void main()
{
	...
	vec3 L = normalize((view * lDir).xyz); //	Light Vec
	vec3 N = normalize(texture(normalMap, vUV).xyz); //				  Normal vec

	vec3 R = reflect(L, N); //										  Reflection vec
	vec3 E = normalize(view[3].xyz + texture(positionMap,vUV).xyz); //View vec
	float sP = 2;
